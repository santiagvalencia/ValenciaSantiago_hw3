...
img_FT = fft2(img)
plt.imshow(np.abs(np.real(img_FT)), norm = LogNorm(vmin = 3), cmap = "hot")
...
for i in range(img_FT_filtro[:, 0].size):
    for j in range(img_FT_filtro[0].size):
        if np.abs(np.real(img_FT[i, j])) >= 10.0**3.3: #si el valor de la transformada es mayor a 10**3.3 este valor pasa a ser el promedio de sus vecinos
            img_FT_filtro[i, j] = (img_FT_filtro[i+1, j] + img_FT_filtro[i-1, j] + img_FT_filtro[i, j+1] + img_FT_filtro[i, j-1])/4
#Guarda el valor absoluto de la parte real de la transformada filtrada
plt.imshow(np.abs(np.real(img_FT_filtro)), norm = LogNorm(vmin = 3), cmap = "hot")
plt.colorbar(label = "Transformada 2D de Fourier")
